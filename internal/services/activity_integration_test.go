...
func TestActivityServiceRejectsInvalidInputs(t *testing.T) {
	...
	if err := svc.Record(ctx, 1, "created", objectref.Ref{Type: objectref.TypeCustomer}, nil); err == nil {
		t.Fatal("Record invalid target error = nil")
	}
	if err := svc.Record(ctx, 1, " \t", objectref.New(objectref.TypeCustomer, 1), nil); err == nil {
		t.Fatal("Record empty action error = nil")
	}
	if _, _, err := svc.ListByType(ctx, objectref.Type("legacy"), 0, 10); err == nil {
		t.Fatal("ListByType unknown type error = nil")
	}
	if _, _, err := svc.ListByTypes(ctx, nil, 0, 10); err == nil {
		t.Fatal("ListByTypes empty types error = nil")
	}
	if _, _, err := svc.ListByTypeAndActions(ctx, objectref.TypeJob, nil, 0, 10); err == nil {
		t.Fatal("ListByTypeAndActions empty actions error = nil")
	}
	if _, _, err := svc.ListByTypeAndActions(ctx, objectref.TypeJob, []string{""}, 0, 10); err == nil {
		t.Fatal("ListByTypeAndActions blank action error = nil")
	}

	svc = NewActivityService(nil, nil)
	if err := svc.Record(ctx, 1, "created", objectref.New(objectref.TypeCustomer, 1), nil); err == nil {
	...
	if _, err := svc.ListForObject(ctx, objectref.New(objectref.TypeCustomer, 1), 10); err == nil {
...
func TestActivityServiceTypedPersistenceAndFilteringIntegration(t *testing.T) {
	...
	if err := svc.Record(ctx, 42, "  created  ", customer, map[string]interface{}{"name": "Missing customer"}); err != nil {
		t.Fatalf("Record missing target: %v", err)
	}
	if err := svc.Record(ctx, 42, "updated", customer, nil); err != nil {
		t.Fatalf("Record customer update: %v", err)
	}
	if err := svc.Record(ctx, 42, "created", job, nil); err != nil {
		t.Fatalf("Record job: %v", err)
	}

	forObject, err := svc.ListForObject(ctx, customer, 10)
	if err != nil {
	...
	created, total, err := svc.ListByTypeAndActions(ctx, objectref.TypeCustomer, []string{"created"}, 0, 10)
	if err != nil {
	...
	byTypes, total, err := svc.ListByTypes(ctx, []objectref.Type{objectref.TypeCustomer, objectref.TypeJob}, 0, 10)
	if err != nil {
...
func TestActivityServiceMapsMalformedLegacyRowsIntegration(t *testing.T) {
	...
	entries, total, err := svc.ListAll(ctx, 0, 10, true)
	if err != nil {

...
func TestTimeEntryListDateFilterUsesCompanyTimezoneAndRetainsValues(t *testing.T) {
	...
	client.TimeEntry.Create().SetUserID(admin.ID).SetClockIn(time.Date(2026, time.July, 10, 3, 59, 59, 0, time.UTC)).SetNotes("previous local day").SaveX(ctx)
	client.TimeEntry.Create().SetUserID(admin.ID).SetClockIn(time.Date(2026, time.July, 10, 4, 0, 0, 0, time.UTC)).SetNotes("local day start").SaveX(ctx)
	client.TimeEntry.Create().SetUserID(admin.ID).SetClockIn(time.Date(2026, time.July, 11, 3, 59, 59, 0, time.UTC)).SetNotes("local day end").SaveX(ctx)
	client.TimeEntry.Create().SetUserID(admin.ID).SetClockIn(time.Date(2026, time.July, 11, 4, 0, 0, 0, time.UTC)).SetNotes("next local day").SaveX(ctx)
...
func TestTimeEntryListEnforcesRoleScopeAndCombinesOfficeFilters(t *testing.T) {
	...
	client.TimeEntry.Create().SetUserID(tech.ID).SetClockIn(clockIn).SetNotes("repair self result").SaveX(ctx)
	client.TimeEntry.Create().SetUserID(other.ID).SetClockIn(clockIn).SetNotes("repair selected result").SaveX(ctx)
	client.TimeEntry.Create().SetUserID(other.ID).SetClockIn(clockIn.AddDate(0, 0, 1)).SetNotes("repair wrong date").SaveX(ctx)
	client.TimeEntry.Create().SetUserID(other.ID).SetClockIn(clockIn).SetNotes("inspection wrong search").SaveX(ctx)
